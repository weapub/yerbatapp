...
export const BackupsTab = () => {
  ...
  const columns: ResponsiveTableColumn<BackupLog>[] = [
    { header: 'Fecha', cell: (b) => new Date(b.iniciadoEn).toLocaleString('es-AR') },
    { header: 'Tipo', cell: (b) => b.tipo },
    { header: 'Estado', cell: (b) => <Badge tone={ESTADO_TONE[b.estado]}>{b.estado}</Badge> },
    { header: 'Tamaño', cell: (b) => formatBytes(b.tamanioBytes) },
    {
      header: '',
      actions: true,
      cell: (b) =>
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card>
        {isLoading ? (
          ...
          <ResponsiveTable data={data} rowKey={(b) => b.id} columns={columns} />
        )}
      </Card>
    </div>
  );
